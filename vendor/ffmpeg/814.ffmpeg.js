!(function (e, t) {
  ...
  (() => {
    ...
    const r = new Error("unknown message type"),
    ...
    let o;
    return (
      (self.onmessage = async ({ data: { id: n, type: E, data: c } }) => {
        const i = [];
        let p;
        try {
          if (E !== t.LOAD && !o) throw a;
          switch (E) {
            case t.LOAD:
              p = await (async ({ coreURL: r, wasmURL: a, workerURL: n }) => {
                const E = !o;
                try {
                  r || (r = e), importScripts(r);
                } catch {
                  if (
                    (r || (r = e.replace("/umd/", "/esm/")),
                ...
                const c = r,
                  i = a || r.replace(/.js$/g, ".wasm"),
                  p = n || r.replace(/.js$/g, ".worker.js");
                return (
                  (o = await self.createFFmpegCore({
                    mainScriptUrlOrBlob: `${c}#${btoa(
                      JSON.stringify({ wasmURL: i, workerURL: p })
                    )}`,
                  })),
                  o.setLogger((e) =>
                    self.postMessage({ type: t.LOG, data: e })
                  ),
                  o.setProgress((e) =>
                    self.postMessage({ type: t.PROGRESS, data: e })
                  ),
                  E
                );
              })(c);
            ...
            case t.LIST_DIR:
              p = (({ path: e }) => {
                const t = o.FS.readdir(e),
                  r = [];
                for (const a of t) {
                  const t = o.FS.stat(`${e}/${a}`),
                    s = o.FS.isDir(t.mode);
            ...
            case t.DELETE_DIR:
              ...
              break;
            case t.MOUNT:
              p = (({ fsType: e, options: t, mountPoint: r }) => {
                const a = e,
                  s = o.FS.filesystems[a];
        ...
        p instanceof Uint8Array && i.push(p.buffer),
          self.postMessage({ id: n, type: E, data: p }, i);
      }),
      {}
    );
  })()
