...
export function setItemMeta(id,info){
  ...
  // Resolution appears first, before FPS, as requested
...
export function wireSaveButtons(id, blob, name, saveWithPicker){
  const a=document.querySelector(`#item-${id} #actions-${id}`) || document.getElementById(`actions-${id}`);
  if(!a) return;
  a.innerHTML='';
