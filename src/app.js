...
(async()=>{
  try{
    // Wait for UMD loader signal (set in index.html)
    if (!window.__FFMPEG_UMD_OK__) {
      // Give it a moment in case index loader is still attaching the script
      for (let i=0;i<50;i++){ if(window.__FFMPEG_UMD_OK__) break; await new Promise(r=>setTimeout(r,100)); }
    }
    ffmpeg=await initFFmpeg();
    ffmpegReady=true; status.textContent='Converter ready. Please add files.'; updateStart();
  }catch(e){ console.error(e); status.textContent='Error loading converter engine. Ensure vendor/ffmpeg/* exists and UMD is correct.'; }
})();
